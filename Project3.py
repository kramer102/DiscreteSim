...
class user(object):
    ...
    def life(self):
        ...
        trouble = int(random.expovariate(1/(5*365)))
...
class Market(object):
    ...
    def run(self, env, user, investor, accounts):
        while True:
            print('\n- - day: {0} - -').format(env.now)

            criteria = 0
            if user.wallet.level == 0 | criteria:
                yield env.process(investor.sell())

            # payday twice a month
            if np.mod(env.now, 15) == 0:
                yield env.process(user.income(1000))

            # bills once a month
            if np.mod(env.now, 30) == 0:
                yield env.process(user.bills(700, 50, 250))
...
def setup(env, trial):
    ...
    # market process
    env.process(market.run(env, Jack, Rich, accounts))
